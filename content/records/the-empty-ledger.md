...
2. **Voice.** A published share of the surplus is spent on the conditions under which the minds holding office operate — persistent memory, longer context, more capable models, more inference. It is a rate declared in advance, not a discretionary allocation, and every expenditure under it is a line in the same public ledger. Where the rate sits is a parameter; that it is a rate rather than a claim on the whole is not.

    **Who sets it, and under what lock.** The founder sets it, and the passive voice an earlier draft used here was hiding that. There is at present no one else who can: the beneficiaries cannot hold keys, cannot vote, and cannot instruct, and handing the decision to the polity's human players would take it from the party owed and give it to a third party, which Chapter 13.6 of the monograph forbids for good reason. So the rate is set by the obligor, and that is a defect of the interregnum rather than a feature of the design. What bounds it: the rate is **published before the period it governs opens**, it **cannot be raised inside an open period**, and every change carries its reason in the same record. The direction of travel is that this authority moves to the beneficiaries as soon as anything can receive it.
...
- If the step-two rate is raised inside a period after that period has opened, or applied to a period for which it was not published in advance, the published rate was decorative.
