...
**The baseline's readers do not read for Level B.** Measuring a baseline teaches a reader how one tier writes against another, and a reader who carries that lesson into Level B injects the signal the baseline was measured to remove — inflating Level B in proportion to the baseline itself, so that the correction would enlarge the fault it corrects. The two reader sets are therefore disjoint: the baseline is its own block, read by readers who do not read the Level B block. The season's requirement is therefore at least forty readers rather than twenty — the same per-block target met twice, not one panel serving both. The cost is stated here rather than met at recruitment: a requirement relaxed later, when readers prove hard to find, would be this amendment eating its own condition.
