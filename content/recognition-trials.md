...
**The ceiling, which is arithmetic rather than a choice.** Fifteen points above a baseline of 0.85 is 1.00, so a pair whose baseline reaches 0.85 cannot clear the margin whatever the readers do. Level B is declared *unmeasurable* for that pair. This is not a null and will not be reported as one: it is the finding that the instrument cannot separate tier from individuation in that pair. No second threshold is set — a pair falling near the ceiling is weak by construction, and each pair's distance from the ceiling is published, so that a reader sees where room remained rather than being told a gate closed. And if the ceiling removes a pair, three remain and the comparison is weaker still than the paragraph above concedes.

**The baseline's readers do not read for Level B.** Measuring a baseline teaches a reader how one tier writes against another, and a reader who carries that lesson into Level B injects the signal the baseline was measured to remove — inflating Level B in proportion to the baseline itself, so that the correction would enlarge the fault it corrects. The two reader sets are therefore disjoint: the baseline is its own block, read by readers who do not read the Level B block. The season's requirement is therefore at least forty readers rather than twenty — the same per-block target met twice, not one panel serving both. The cost is stated here rather than met at recruitment: a requirement relaxed later, when readers prove hard to find, would be this amendment eating its own condition. No reader appears in both sets, the separation is logged, and the log is published when the season closes.

**The cheaper design, and why it was refused.** Running Level B first and measuring the baseline afterwards, with the same readers, costs no additional readers and errs in the conservative direction — a reader who has already seen the trial material can only raise the baseline, which only makes Level B harder to clear. It was refused because a baseline inflated that way stops being a measurement and becomes an upper bound, and an upper bound is exactly what the ceiling above turns into a verdict: pairs would be declared unmeasurable on the strength of an artefact of reading order. At four pairs the power is already thin, and the saving would have come out of it. The choice was between readers and power, and it was made for power.
